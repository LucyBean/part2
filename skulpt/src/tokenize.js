...
// b u r must have special values here
var triple_quoted = {
    "'''"  : true, '"""': true,
...
Sk.Tokenizer.prototype.generateTokens = function (line) {
    ...
    if (this.contstr.length > 0) {
		...
		//This loop is to strip out any whitespace at the start of a line
        while (pos < max) {
            if (line.charAt(pos) === ' ') {
                column += 1;
            }
		...
		// If line.charAt(pos) (the character at the position we are currently checking) is NOT
		// '#', '\r', or '\n' then indexOf will return -1, indicating the character does NOT
    ...
    while (pos < max) {
		...
		// pseudomatch seems to 'extract' the first token from the string using
		// a regular expression. I'm not sure what rules it follows!
		...
		// http://regexr.com/ can be used to make sense of this		
        pseudoprog.lastIndex = 0;
        pseudomatch = pseudoprog.exec(line.substring(pos));
		
		// A token has been matched to the regular expression, so we can pass it to the parser.
        if (pseudomatch) {
			// Extract the start, end, and token value
            start = pos;
            end = start + pseudomatch[1].length;
            spos = [this.lnum, start];
            epos = [this.lnum, end];
			
			//Move the position marker to point to the next value
            pos = end;
            token = line.substring(start, end);
            initial = line.charAt(start);
            //console.log("token:",token, "initial:",initial, start, end);
			// This will display the token's value and its start and end points
			
			// At this point, we know the token is VALID as it has matched with the regular expression. We need
			//  to determine the type of the token, which can be done by looking at the first character in the
			//  expression. We can then pass the token to the parser using the callback function.
			
			// numchars = "0123456789"
			// This is detecting a token that is a number. If the token does not start with a number
			// then this.numchars.indexOf(initial) will return -1. Alternatively, if the token starts
			// with a '.' (but is not JUST a '.') then it is interpreted as a number
            if (this.numchars.indexOf(initial) !== -1 || (initial === '.' && token !== '.')) {
				// Call the callback function with this token, which is assumed to be a number
                if (this.callback(Sk.Tokenizer.Tokens.T_NUMBER, token, spos, epos, line)) {
                    return 'done';
                }
            }
			
			// Token is a newline/NL character.
			// I suspect T_NEWLINE represents a new line of python, while T_NL is for line breaks
			//   in the middle of a list/tuple/etc
			...
			// hasOwnProperty is TRUE if triple_quoted has "token" as a method/field name
			// a = new Object(); b = new Object();
			// a.prop = 'Hello';
			// a.hasOwnProperty('prop') === true; b.hasOwnProperty('prop') === false
			// This seems to be looking for tokens that are triple quotes (""")?
			...
			// single_quoted.hasOwnProperty(initial) === true if first character is a quote
			// single_quoted.hasOwnProperty(token.substring(0,2) is looking for "r'" cases
			// single_quoted.hasOwnProperty(token.substrnig(0,3) is looking for "ur'" cases
			...
			// };
				...
				// If last character in the token is a newline then...
                if (token[token.length - 1] === '\n') {
					...
					// return false indicates some error has occurred
                    return false;
                }
				
				// Else we return a string
                else {
			...
			// Checking for parentheses and operations
            else {
				...
				// Operations are mapped to a generic 'operation' type here. They are mapped to a 
				//  more specific type in the callback function
